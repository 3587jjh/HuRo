...
# Arm segmentation (stage 6): detectron2 person detection + SAM2 video segmentation.
...
# Arm inpainting (stage 8): ProPainter.
PROPAINTER_DIR = SUBMODULES / "propainter"
PROPAINTER_WEIGHTS_DIR = PROPAINTER_DIR / "weights"

# Robot retargeting (stage 9): PyRoKi over the robot's URDF. Each robot's config gives the
# URDF path as `robot_urdf_path` (repo-root relative). See robot_urdf().
PYROKI_DIR = SUBMODULES / "pyroki"

# Overlay rendering (stage 10): the URDF is imported once into an Isaac-ready USD, cached
...
def partition(items, part):
    """`--part 'a/b'` split: every b-th item starting at index (a-1)."""

#!/bin/bash
# Run the whole pipeline over one clip directory: stage 1 -> 11, in order.
#
#   ./run_pipeline.sh
#
...
#
# Each GPU in GPUS gets a worker over its own slice of the clips. Stage 11 needs every clip
# at once, so the workers join after stage 10 and one process then converts them all.
#
# Outputs land beside INPUT_DIR (all git-ignored):
#   <clips>_intr  _contact  _contact_refined  _hand  _extr  _arm  _chunked  _lerobot
#
...
ROBOT="allex"                # robot config for stages 9-11 (configs/<ROBOT>.yaml)
FIRST_STAGE=1                # run stages FIRST_STAGE..LAST_STAGE
LAST_STAGE=11
PROGRESS=0                   # 1 keeps the tqdm bars, and 0 passes --no_tqdm (log-friendly)

# Optional per-stage flag overrides, keyed by stage number. Each stage script's header
# documents what it accepts.
declare -A STAGE_ARGS=(
  # [7]="--stride 5 --n_caption_samples 4"
)
...
SCRIPTS=(
  ...
  [5]=stage5_annot_extrinsics.py       [6]=stage6_annot_segment.py
  [7]=stage7_annot_narr.py             [8]=stage8_annot_inpaint.py
  [9]=stage9_robot_retarget.py         [10]=stage10_robot_overlay.py
  [11]=stage11_lerobot_convert.py
)
SHARDED_LAST_STAGE=10   # the last stage that may run on more than one worker

[ -d "$INPUT_DIR" ] || { echo "INPUT_DIR is not a directory: $INPUT_DIR"; exit 1; }
# Only stages 1-7 read the source videos. From stage 8 on, the work comes out of <clips>_chunked.
if [ "$FIRST_STAGE" -le 7 ]; then
...
[ "$FIRST_STAGE" -ge 1 ] && [ "$LAST_STAGE" -le 11 ] && [ "$FIRST_STAGE" -le "$LAST_STAGE" ] \
  || { echo "bad stage range: $FIRST_STAGE..$LAST_STAGE"; exit 1; }

# Stage 10 renders through Isaac Sim, which is proprietary: setting OMNI_KIT_ACCEPT_EULA=Y
# accepts the NVIDIA Omniverse License Agreement, so this script does not set it.
# Checked here so a run that includes stage 10 fails now rather than after stages 1-9.
if [ "$FIRST_STAGE" -le 10 ] && [ "$LAST_STAGE" -ge 10 ] && [ "${OMNI_KIT_ACCEPT_EULA:-}" != "Y" ]; then
  echo "stage 10 renders through Isaac Sim, licensed under the NVIDIA Omniverse License Agreement"
  echo "(site-packages/isaacsim/LICENSE.txt). Read it, then accept with"
  echo
  echo "    export OMNI_KIT_ACCEPT_EULA=Y"
  echo
  echo "and re-run. To stop before the overlay instead, set LAST_STAGE=9."
  exit 1
fi
...
vulkan_icd_env() {
  ...
  echo "   vulkan: no NVIDIA ICD manifest found, so stage 10 will report what is missing"
}
...
isaac_env() {
  ...
  # refuses to start stage 10 until it is set.
...
run_stage() {  # run_stage <stage-number> <part>
  ...
  case $n in 9|10|11) args+=(--robot_name "$ROBOT") ;; esac

  echo ""
  echo "== stage $n: ${SCRIPTS[$n]}  (part $part, $(date '+%F %T')) =="
  # subshell: stage 10's Isaac Sim environment must not leak into later stages
  (
    if [ "$n" = 10 ]; then isaac_env; fi
...
# Stage 7's VLM (~18 GB) downloads on first use. Fetch it once rather than once per worker.
if [ "$N" -gt 1 ] && [ "$FIRST_STAGE" -le 7 ] && [ "$LAST_STAGE" -ge 7 ]; then
  echo ""
  echo "== stage 7 VLM: warming the HF cache before fanning out =="
...
# Stages 1-10: one worker per GPU, each over its own slice of the clips.
if [ "$FIRST_STAGE" -le "$SHARDED_LAST_STAGE" ]; then
  if [ "$N" -eq 1 ]; then
    ...
    [ "$FAILED" -eq 0 ] || { echo ""; echo "== a worker failed, not starting stage 11 =="; exit 1; }
  fi
fi

# Stage 11 sees every clip at once, so it runs once, after every worker has finished.
if [ "$LAST_STAGE" -ge 11 ]; then
  export CUDA_VISIBLE_DEVICES="${GPUS[0]}"
  run_stage 11 "1/1"
fi
